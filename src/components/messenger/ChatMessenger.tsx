...
import { ReactComponent as SendIcon } from "../../assets/icons/icon_send diagonal_.svg";
  ...
  return (
      <div className="w-2/3 bg-white pt-5 px-4 rounded-lg  flex flex-col h-full overflow-y-auto">
        <div className="pb-2 mb-4">
          <div
            className={`flex items-center space-x-3 pb-4 ${selectedChat ? "border-b border-purple-300" : "border-none "
              }`}
          >
            {selectedChat && (
            ...
            {selectedChat && (
              <div className="space-y-2">
                {selectedChat.messages.map((message) => (
                  <div
                    key={message.id}
                    className={`p-2 rounded-md shadow-md w-96 text-white ${!message.isManager
                      ? "bg-[#9747FF80] ml-auto"
                      : "bg-[#D3ABFFB2] mr-auto"
                      }`}
                  >
